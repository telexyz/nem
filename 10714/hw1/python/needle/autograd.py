...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    # thay vì vector output_node. Nhưng đây là trường hợp phổ biến đối với hàm mất mát (loss function).
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    # print(" >>>> ", node_list) # => needle.Tensor([[4.63946401]])
    # node = node_list[0];
    # print(" >>>> ", node.op, node.inputs, node.cached_data, node.requires_grad)
    # => <needle.ops.EWiseAdd object at 0x1124fedd0> (needle.Tensor([[5.52228558]]), needle.Tensor([[-0.88282157]])) [[4.63946401]] True
    
    ### BEGIN YOUR SOLUTION
    topo_order = []
    for node in node_list:
        topo_sort_dfs(node, topo_order, topo_order)
    return topo_order
    ### END YOUR SOLUTION
